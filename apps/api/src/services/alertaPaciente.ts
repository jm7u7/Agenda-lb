...
export function evaluarAlerta(noShows: number, reprogramaciones: number): AlertaPaciente {
...
export async function alertasDePacientes(pacienteIds: (string | null | undefined)[]): Promise<Map<string, AlertaPaciente>> {
  ...
  // 1) No-shows por paciente.
  const ns = await prisma.cita.groupBy({
    by: ['pacienteId'],
    where: { pacienteId: { in: ids }, estado: 'no_show' as never, deletedAt: null },
    _count: { _all: true },
  });
  const nsMap = new Map(ns.map((r) => [r.pacienteId, r._count._all]));
  ...
  for (const id of ids) {
    map.set(id, evaluarAlerta(nsMap.get(id) ?? 0, repMap.get(id) ?? 0));
  }
  return map;
}
